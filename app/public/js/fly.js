/*
 * :file description:
 * :name: /digital/app/public/js/fly.js
 * :author: 张德志
 * :copyright: (c) 2023, Tungee
 * :date created: 2023-06-06 20:04:54
 * :last editor: 张德志
 * :date last edited: 2023-06-06 20:08:44
 */
...
function currentCityAllFlys(start, endArr) {
  ...
  // 标注发射中心
  const startPoint = cityPoint(start);
  ...
  endArr.forEach((cood, i) => {
    const end = new THREE.Vector3(cood.E, cood.N, 0); // 终点
    const cityPointMesh = cityPoint(end);
    cityGroup.add(cityPointMesh);
    cityPointMesh.s = _s * Math.random() + 1; // 随机设置一个缩放倍数
    cityPointArr.push(cityPointMesh);
    // 飞线运动轨迹绘制函数flyTrack
    const flyTrack = flyTrackFun(start, end);
    flyGroup.add(flyTrack); // 线条对象添加到场景中

    // 获取飞线轨迹线上的顶点坐标，用于飞线段绘制
    const points = flyTrack.flyTrackPoints;

    const index = 20; // 飞线索引起点
    const flyline = flylineFun(index, points); // 绘制一段飞线
    ...
    const maxH = 4; // 假设最大值
    const h = 1 + (maxH - 1) * Math.random();
    const heightMesh = cityHeight(h, h / maxH);
    // 通过经纬度坐标设置柱子在地图上位置
...
function startMeshFun(startCoord) {
  const startgeometry = new THREE.ConeBufferGeometry(0.5, 2, 4);
...
const geometryHeight = new THREE.CylinderGeometry(0.2, 0.2, 1, 6);
...
const color2 = new THREE.Color(0x00ff33);
function cityHeight(h, percent) {
  ...
  const mesh = new THREE.Mesh(geometryHeight, material);
...
cityCoord：表示城市经纬度坐标*/
const cityPointTexture = texLoad.load('https://tugua.oss-cn-hangzhou.aliyuncs.com/model/point.png');
function cityPoint(cityCoord) {
  ...
  const cirMesh = new THREE.Mesh(geo, mat);
  cirMesh.position.copy(cityCoord);
  return cirMesh;
}
// updateFlyGeo：更新飞线几何体顶点位置坐标
function updateFlyGeo(flyline, index, points) {
  const pointArr = []; // 存储飞线轨迹上选择的顶点坐标，用于飞线绘制
  for (let i = 0; i < flyline.num; i++) {
    const v3 = points[i + index];
...
function flylineFun(index, points) {
  const choosePoints = []; // 存储飞线轨迹上选择的顶点坐标，用于飞线绘制
  const num = 11; // 从曲线上取11个点 飞线长度占飞线轨迹长度的10%  你可通过获取的点数调节飞线长度
  for (var i = 0; i < num; i++) {
  ...
  const flyline = new THREE.Line2(geometry, material);
...
function flyTrackFun(start, end) {
  ...
  const H = length * 0.1; // 根据两点之间距离设置弧线高度
  ...
  const points = curve.getPoints(100); // 分段数100，返回101个顶点，返回一个vector3对象作为元素组成的数组
